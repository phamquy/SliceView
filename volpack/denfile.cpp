...
* denfile.c
*
*  MODIFIED FOR WINDOWS BY JOE ZADEH
*/
...
read_bytes(FILE *fd, char *buf, int bytecount)
{
    int n;
	
    while (bytecount > 0) {
		n = MIN(bytecount, MAX_READ_SIZE);
		if (fread(buf, sizeof(char), n, fd) != n)
			return(0);
		buf += n;
		bytecount -= n;
    }
...
read_shorts(FILE *fd, short *sbuf, int shortcount, int swap)
{
    int n, c;
...
read_words(FILE *fd, int *wbuf, int wordcount, int swap)
{
    int n, c;
...
write_bytes(FILE *fd, char *buf, int bytecount)
{
    int n;
