...
static int StoreRLEVoxels ANSI_ARGS((vpContext *vpc, FILE *fd,
    RLEVoxels *rle_voxels));
static int LoadRLEVoxels ANSI_ARGS((vpContext *vpc, FILE *fd,
...
static int StoreTable ANSI_ARGS((vpContext *vpc, FILE *fd, float *ptr,
    unsigned size));
static int LoadTable ANSI_ARGS((vpContext *vpc, FILE *fd, float **ptr_ptr,
    unsigned *size_ptr));
...
FILE *fd;		/* UNIX file descriptor open for writing */
{
    RLEVoxelHdr header;
...
if (fwrite(&header, sizeof(char), sizeof(header), fd) != sizeof(header))
    ...
    if (fwrite(field_data, sizeof(char), size, fd) != size)
	return(VPSetError(vpc, VPERROR_IO));
  ...
  if (fwrite(pad_data, sizeof(char), pad_bytes, fd) != pad_bytes)
	    return(VPSetError(vpc, VPERROR_IO));
    }
...
RLEVoxels *rle_voxels;
{
	...
	if (fwrite(rle_voxels->run_lengths, sizeof(char), size, fd) != size)
	    return(VPSetError(vpc, VPERROR_IO));
	...
	if (fwrite(rle_voxels->data, sizeof(char), size, fd) != size)
	    return(VPSetError(vpc, VPERROR_IO));
	...
	if (fwrite(rle_voxels->scan_offsets, sizeof(char), size, fd) != size)
	    return(VPSetError(vpc, VPERROR_IO));
...
FILE *fd;		/* WINDOWS file descriptor open for reading */
{
    RLEVoxelHdr header;
    unsigned field_data[3*VP_MAX_FIELDS];
    int nsf, c, swab;
	...
	if (fread(&header, sizeof(char), sizeof(header), fd) != sizeof(header)) 
	return(VPSetError(vpc, VPERROR_IO));
    ...
    size = 3 * header.num_shade_fields * sizeof(unsigned);
  	if (fread(field_data, sizeof(char), size, fd) != size) 
	return(VPSetError(vpc, VPERROR_IO));
	...
	if (fread(pad_data, sizeof(char), pad_bytes, fd) != pad_bytes)
	    return(VPSetError(vpc, VPERROR_IO));
    }
...
int swab;
{
	...
	if (fread(rle_voxels->run_lengths, sizeof(char), size, fd) != size)
	    return(VPSetError(vpc, VPERROR_IO));
	...
	if (fread(rle_voxels->data, sizeof(char), size, fd) != size)
	    return(VPSetError(vpc, VPERROR_IO));
	...
	if (fread(rle_voxels->scan_offsets, sizeof(char), size, fd) != size)
	    return(VPSetError(vpc, VPERROR_IO));
...
FILE *fd;		/* UNIX file descriptor open for writing */
{
    ...
    size = vpc->mm_octree->octree_bytes;
    	if (fwrite(vpc->mm_octree->root, sizeof(char), size, fd) != size)
	return(VPSetError(vpc, VPERROR_IO));
...
FILE *fd;		/* WINDOWS file descriptor open for reading */
{
	...
	if (fread(vpc->mm_octree->root, sizeof(char), size, fd) != size)
	return(VPSetError(vpc, VPERROR_IO));
    if (swab)
...
FILE *fd;		/* UNIX file descriptor open for writing */
{
	...
	if (fwrite(vpc->raw_voxels, sizeof(char), vpc->raw_voxels_size, fd) != vpc->raw_voxels_size)
	return(VPSetError(vpc, VPERROR_IO));
...
FILE *fd;		/* WINDOWS FILES!*/
{
	...
	if (fread(vpc->raw_voxels, sizeof(char), size, fd) != size)
	    return(VPSetError(vpc, VPERROR_IO));
	if (swab) {
...
FILE *fd;
{
	...
	if (fwrite(&header, sizeof(char), sizeof(header), fd) != sizeof(header))
		...
		if (fwrite(vpc, sizeof(char), vpc_size, fd) != vpc_size)
		return(VPSetError(vpc, VPERROR_IO));
...
unsigned size;
{
    if (size == 0 || ptr == NULL) {
		...
		if (fwrite(&size, sizeof(char), sizeof(size), fd) != sizeof(size))
			return(0);
    } else {
		...
		if (fwrite(ptr, sizeof(char), size, fd) != size)
			return(0);
    }
    return(1);
}
...
FILE *fd;
{
	...
	if (fread(vpc, sizeof(char), vpc_size, fd) != vpc_size)
	return(VPSetError(vpc, VPERROR_IO));
    vpc->raw_voxels = NULL;
...
unsigned *size_ptr;
{
	...
	if (fread(size_ptr, sizeof(char), sizeof(unsigned), fd) != sizeof(unsigned))
	return(0);
    if (*size_ptr != 0) {
	Alloc(vpc, *ptr_ptr, void *, *size_ptr, "lookup table");
	
	if (fread(*ptr_ptr, sizeof(char), *size_ptr, fd) != *size_ptr)
	    return(0);
    }
    return(1);
}
