...
vpResult
vpWindow(vpc, type, left, right, bottom, top, nearn, farf)
vpContext *vpc;
int type;
double left, right, bottom, top, nearn, farf;
{
    vpMatrix4 projectm, tmp;

    if (left >= right || bottom >= top || nearn >= farf)
	return(VPSetError(vpc, VPERROR_BAD_VALUE));
	...
	projectm[2][2] = 2. / (farf - nearn);
	projectm[0][3] = (left + right) / (left - right);
	projectm[1][3] = (bottom + top) / (bottom - top);
	projectm[2][3] = (nearn + farf) / (nearn - farf);
    } else {
	/* XXX perspective rendering not available yet */
	return(VPSetError(vpc, VPERROR_BAD_OPTION));
#ifdef notdef
	projectm[0][0] = 2. * nearn / (right - left);
	projectm[1][1] = 2. * nearn / (top - bottom);
	projectm[2][2] = (nearn + farf) / (nearn - farf);
	projectm[0][2] = (right + left) / (right - left);
	projectm[1][2] = (top + bottom) / (top - bottom);
	projectm[3][2] = -1.;
	projectm[2][3] = 2. * farf * nearn / (nearn - farf);
#endif
    }
    if (vpc->concat_left)
			   ...
			   (VP_IMAGE_BUFFER or VP_SHADOW_BUFFER) */
{
	...
	case VP_ALPHA:
		...
		for (y = 0; y < height; y++) {
			...
			//value = (int)gray_pixel->opcflt * 255;
		...
		for (y = 0; y < height; y++) {
			...
			//	value = (int)rgb_pixel->opcflt * 255.;
			if (value > 255)
			    *dst_ptr = 255;
			else
			    *dst_ptr = value;
